...
**Post-hoc correction (2026-09-05 final-review fix, same precedent as `b69a11a`):** the original draft's `setup()` called `locoNetPort.begin()`; that call is removed above because `LocoNetEsp32Port::begin()` became a documented no-op in the Task 7 follow-up fix (see that task's correction note) — the vendor's constructor already performs hardware init.
