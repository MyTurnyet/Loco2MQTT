...
- **Decided during Task 4 review, binding on every remaining task:** the project's "methods ≤ 8 lines" rule does not require splitting a pure dispatch function (a switch or if-chain where every branch is one line delegating to a same-named helper or factory, with no nested logic) purely to hit the line count — `CommandLineParser::parseCommandLine` (Task 3) and `CommissioningSession::apply` (Task 4) are both accepted as-is under this reading. The rule still applies in full to every method with actual logic in its body.
